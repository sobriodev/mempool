 ...
 * If the library is compiled with extra DLL_HEAD_SANITY_CHECK flag additional check is executed on the head node.
 * This means pointer to a previous node must be set to NULL, therefore preventing from destroying only part of the list
 * when middle node was passed instead of a head.
 *
 * @param head Pointer to a head node.
 * @param decay_fn Decay function pointer.
 * @return The function returns:
 *         dll_status_iptr when NULL pointer was passed in place of head node or decay function
 *         dll_status_inv_node if sanity check failed (if enabled)
 ...
 *
 * Inserting whole lists is not supported by this function.
 *
